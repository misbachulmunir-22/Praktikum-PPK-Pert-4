...
export default function Home() {
  ...
  // Check auth session & preferences on mount
  useEffect(() => {
    checkAuth();
  }, []);
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setAuthError(data.error || "Terjadi kesalahan.");
        return;
      }
      ...
      setAuthError("Gagal menghubungi server. Pastikan koneksi aman.");
    } finally {
  ...
  const handleDemoLogin = async () => {
    setAuthEmail("budi@students.undip.ac.id");
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAuthError(data.error || "Gagal login demo.");
      }
    } catch {
      setAuthError("Gagal melakukan login demo.");
    } finally {
      setAuthSubmitting(false);
    }
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-900 text-white">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-400 font-medium">Memuat Expense Tracker...</p>
        </div>
      </div>
    );
  }

  // --- UNAUTHENTICATED: REGISTER & LOGIN SCREEN ---
  if (!user) {
    return (
        ...
        <div className="w-full max-w-md space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-slate-400">
              Kelola uang saku, beasiswa, dan pengeluaran harian mahasiswa secara mudah & praktis.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-2xl backdrop-blur-xl sm:p-8">
            {/* Tabs Toggle */}
            <div className="grid grid-cols-2 gap-1 rounded-xl bg-slate-800/80 p-1 mb-6">
              <button
                type="button"
                onClick={() => {
                  setAuthMode("login");
            ...
            <div className="mt-6 border-t border-slate-800 pt-4 text-center">
              <p className="text-xs text-slate-400 mb-2">Ingin cepat mencoba?</p>
              <button
                type="button"
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Keuangan Mahasiswa</p>
            </div>
          </div>
